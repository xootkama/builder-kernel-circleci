...
if [ ! -z "$1" ] && [ "$1" == 'initial' ];then
    ...
    if [ ! -z "$2" ] && [ "$2" == 'full' ];then
        ...
        git clone https://$GIT_SECRET@github.com/$GIT_USERNAME/begonia_kernel -b "$branch" $kernelDir
    else
        getInfo ">> cloning kernel . . . <<"
        git clone https://$GIT_SECRET@github.com/$GIT_USERNAME/begonia_kernel -b "$branch" $kernelDir --depth=1 
    fi
